...
open SlimCheck in
/- Patched for Lean v4.31.0 (was `instance`): a non-inferrable `cfg`
argument is a hard error on newer toolchains, and the only use site applies
this explicitly. See vendor/LSpec/LSPEC_COMMIT for upstream provenance. -/
@[reducible] def instTestableOfCheckable (p : Prop) (cfg : Configuration) [Checkable p] : Testable p :=
